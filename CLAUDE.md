...
- **设计先行**：设计文档和开发计划必须走在代码开发前面。每个版本迭代前，先完成里程碑文档（功能清单、文件规划、技术方案），经确认后再开始编码
...
- **变更文档**：每次版本迭代完成后，必须更新以下变更记录文档：
  - `CHANGELOG.md`（项目根目录）：按 [Keep a Changelog](https://keepachangelog.com/zh-CN/) 格式，记录新增/变更/修复/移除等分类变更
  - `docs/milestones/vX.Y.md`：对应版本的里程碑详细文档，包含版本目标、功能清单、新增/修改文件列表、测试报告
  - 变更记录应包含：功能描述、涉及的关键文件、技术实现要点、发布信息（版本号/日期/标签/分支/提交哈希）
